"""
MGSAN model compatible with PyTorch 2.x and CUDA 12.x (Blackwell/RTX 50 series)

Changes from original:
1. Replace einsum with explicit matmul for better GPU compatibility
2. Replace deprecated .new() with torch.zeros()
3. Add device-agnostic tensor creation
4. Fix potential memory alignment issues
"""
import torch
...
class GSA_block(nn.Module):
    """Graph Self-Attention block - CUDA 12.x compatible version."""
    ...
    def __init__(self, in_channels, out_channels, A, heads=3, timedim=64):
        ...
        for i in range(self.heads):
            self.to_q.append(
                nn.Sequential(
                    nn.Conv2d(in_channels, dim_head, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1)),
        ...
        # Register as buffer instead of parameter for better CUDA handling
    ...
    def forward(self, x):
        y = None
        for i in range(self.heads):
            q = self.to_q[i](x)  # (B, dim_head, 1, V)
            k = self.to_k[i](x)  # (B, dim_head, 1, V)
            v = self.to_v[i](x)  # (B, out_channels, T, V)

            # Replace einsum with explicit operations for better CUDA 12.x compatibility
            # Original: dots = self.soft(einsum('b c t i, b c t j -> b c i j', q, k))
            # q: (B, C, 1, V_i) -> squeeze -> (B, C, V_i)
            # k: (B, C, 1, V_j) -> squeeze -> (B, C, V_j)
            # dots: (B, C, V_i, V_j)
            q_squeezed = q.squeeze(2)  # (B, C, V)
            k_squeezed = k.squeeze(2)  # (B, C, V)

            # Compute attention: (B, C, V, 1) @ (B, C, 1, V) -> (B, C, V, V)
            dots = torch.matmul(q_squeezed.unsqueeze(-1), k_squeezed.unsqueeze(-2))  # (B, C, V, V)
            dots = self.soft(dots)

            dots = self.to_att[i](dots) * self.alpha + self.PA[i].unsqueeze(0).unsqueeze(0)

            # Replace einsum with explicit operations
            # Original: out = einsum('b c i j, b c t j -> b c t i', dots, v)
            # dots: (B, out_channels, V_i, V_j)
            # v: (B, out_channels, T, V_j)
            # out: (B, out_channels, T, V_i)
            B, C_out, T, V = v.shape
            # v: (B, C, T, V) -> (B, C, V, T) for matmul
            v_t = v.permute(0, 1, 3, 2)  # (B, C, V, T)
            # dots @ v_t: (B, C, V, V) @ (B, C, V, T) -> (B, C, V, T)
            out = torch.matmul(dots, v_t)  # (B, C, V, T)
...
class Model(nn.Module):
    """MGSAN Model - CUDA 12.x compatible version."""
    ...
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.K = K
        if K != 0:
            I = np.eye(num_point)  # Use num_point instead of hardcoded 25
            self.register_buffer('A_k', torch.from_numpy(
                I - np.linalg.matrix_power(self.graph.A_outward_binary, K)
            ).float())
    ...
    def forward(self, x):
        ...
        if self.Is_joint:
            # Use torch.zeros instead of deprecated .new()
            dif1 = x[:, :, 1:] - x[:, :, 0:-1]
            dif1 = torch.cat([torch.zeros(N * M, C, 1, V, device=x.device, dtype=x.dtype), dif1], dim=2)

            dif2 = x[:, :, 2:] - x[:, :, 0:-2]
            dif2 = torch.cat([torch.zeros(N * M, C, 2, V, device=x.device, dtype=x.dtype), dif2], dim=2)

            dif3 = x[:, :, :-1] - x[:, :, 1:]
            dif3 = torch.cat([dif3, torch.zeros(N * M, C, 1, V, device=x.device, dtype=x.dtype)], dim=2)

            dif4 = x[:, :, :-2] - x[:, :, 2:]
            dif4 = torch.cat([dif4, torch.zeros(N * M, C, 2, V, device=x.device, dtype=x.dtype)], dim=2)
...
if __name__ == '__main__':
    ...
    img = torch.randn(2, 2, 64, 25, 1).to(device)
    net = Model(num_class=7, num_point=25, num_person=1, in_channels=2,
                graph='graph.ntu_rgb_d.Graph').to(device)
    x = net(img)
