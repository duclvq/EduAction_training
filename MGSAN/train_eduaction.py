#!/usr/bin/env python
"""
Training script for MGSAN on EduAction dataset
Windows-compatible version (no resource module dependency)
"""
...
# Import feeders and graph
import feeders.feeder_eduaction
import graph.eduaction
...
def get_parser():
    parser = argparse.ArgumentParser(description='MGSAN for EduAction')
    parser.add_argument('--work-dir', default='./work_dir/eduaction/mgsan_joint',
                        help='the work folder for storing results')
    parser.add_argument('-model_saved_name', default='MGSAN')
    parser.add_argument('--config', default='./config/eduaction/default.yaml',
...
class Processor():
    """Processor for Skeleton-based Action Recognition on EduAction"""
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            if torch.cuda.is_available():
                weights = OrderedDict([[k.split('module.')[-1], v.cuda(output_device)]
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            process.set_description(f'Loss: {loss.data.item():.4f} Acc: {acc.data.item():.4f}')
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            ...
            if type(self.arg.device) is list and len(self.arg.device) > 1:
                if torch.cuda.is_available():
                    weights = OrderedDict([['module.' + k, v.cuda(self.output_device)]
                                          for k, v in weights.items()])
            self.model.load_state_dict(weights)
