...
def eval():
    total = true  = 0 
    POSE_PATH = r"mgsan_predictions/mgsan_online"
    video_test_list = load_text_file(file_path, POSE_PATH)
    s = 0
    saving_folder = "mgsan_csv_results"
    gt_saving_folder = "mgsan_gt_csv_results"
    for video in tqdm(video_test_list):
        ...
        # ws = 30
        pred_label_list  = np.concatenate((np.zeros(30), pred_label_list), axis=0)
        ...
        for i in range(1, len(pred_label_list)):
            if pred_label_list[i] != pred_label_list[i-1]:
                start_list.append(i)
                stop_list.append(i)
                label_list.append(int(pred_label_list[i-1]))
        stop_list.append(len(pred_label_list))
        label_list.append(int(pred_label_list[-1]))
